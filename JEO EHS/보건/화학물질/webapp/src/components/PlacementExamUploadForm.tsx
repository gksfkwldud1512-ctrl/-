...
export function PlacementExamUploadForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/placement-exam/upload", { method: "POST", body: formData });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body.error ?? "업로드에 실패했습니다.");
      }
      const { id } = await res.json();
      router.push(`/placement-exam/${id}/review`);
    } catch (err) {
