...
async function createTemplate(outputPath, dataRows = []) {
  ...
  guide.value = '★ 업체명은 BOS 거래내역서의 업체명과 정확히 일치해야 합니다. 대표자명은 세금계산서 성명란에 사용됩니다. 주소·업태·종목은 세금계산서 일괄발행 시 사용됩니다.';
  ...
  note.value = '출력방법: 유종별 / 판매일자순 / 차량별-판매일자순 / 차량별-유종별 (비워두면 기본)  |  세금계산서발행구분: 합산 / 분리 (비워두면 합산)';
  ...
  const rows = isExample ? [
    ['태성화학(주)',  '123-45-67890', '홍길동', '홍담당', 'example@company.com', '010-1234-5678',
     '충북 음성군 대소면 ○○로 123', '제조업', '화학제품', '차량별-유종별', '합산'],
    ['(주)대한상사', '234-56-78901', '김대표', '김담당', 'daehan@email.com', '031-123-4567',
     '경기도 성남시 ○○구 ○○로 456', '도매업', '연료', '유종별', '분리'],
  ] : dataRows;
...
if (require.main === module) {
  ...
  try {
    ...
    raw.slice(headerIdx + 1).forEach(r => {
      ...
      // 구양식 세금계산서발행방식(통합/분리) → 새양식 세금계산서발행구분(합산/분리)
      ...
      dataRows.push([
        ...
        '',  // 대표자명 — 신규 필드, 사용자가 직접 입력
        String(r[col('담당자명')] || '').trim(),
        String(r[col('이메일')]   || '').trim(),
