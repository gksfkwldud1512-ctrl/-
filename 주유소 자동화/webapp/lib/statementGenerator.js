...
// ─── 시트 내용 작성 (워크북에 시트 1개 추가) ─────────────────────
function addSheet(wb, sheetName, cust, customer, issueDate, year, month, printMethod) {
...
async function createStatement(cust, customer, outputDir, issueDate, year, month, printMethod) {
  ...
  if (printMethod === '유종별 시트분리') {
    const groups = organizeTransactions(cust.txs, '유종별');
    for (const g of groups) {
      addSheet(wb, g.label, { ...cust, txs: g.txs }, customer, issueDate, year, month, '판매일자순');
    }
  } else {
    addSheet(wb, '거래명세서', cust, customer, issueDate, year, month, printMethod);
  }
