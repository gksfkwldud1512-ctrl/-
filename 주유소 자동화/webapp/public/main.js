...
// ── 상태 ─────────────────────────────────────────────────────
const PRINT_METHODS = ['유종별', '유종별 시트분리', '판매일자순', '차량별-판매일자순', '차량별-유종별'];
...
function initFileUpload() {
  ...
  document.getElementById('delivery-file-input').addEventListener('change', e => {
    if (e.target.files[0]) uploadDelivery(e.target.files[0]);
  });
...
async function uploadDelivery(file) {
  ...
  try {
    const res  = await fetch('/api/upload-delivery', { method: 'POST', body: form });
    const data = await res.json();
    if (data.ok) {
      label.textContent = `✅ ${data.count}건 배달 등록`;
      state.vendors = data.vendors;
      const errorCount = data.vendors.filter(v => v.hasError).length;
      toast(`✅ 배달 거래내역 ${data.count}건 매칭 완료`, 'success');
      if (errorCount > 0) {
        setTimeout(() => toast(`⚠ ${errorCount}개 업체에서 차량번호 없는 거래가 있습니다.`, 'warn'), 800);
      }
      renderAll();
    } else {
      label.textContent = '업로드 실패';
...
function showErrorModal(name) {
  ...
  const noVehicleErrors = vendor.errors.filter(e => e.type === 'no_vehicle');
  ...
  if (noVehicleErrors.length) {
    body += `<p class="error-section-title" style="margin-top:18px;">3. 차량번호 없는 외상 거래</p>
    <table class="error-table">
      <thead><tr><th>날짜</th><th>유종</th><th>주유량</th><th>금액</th></tr></thead>
      <tbody>${noVehicleErrors.map(e => `
        <tr>
          <td>${esc(e.date)}</td>
          <td>${esc(e.product)}</td>
          <td>${Number(e.qty).toLocaleString()}L</td>
