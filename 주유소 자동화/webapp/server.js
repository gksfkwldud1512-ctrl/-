...
function deliveryFile(year, month) {
  const mo = String(month).padStart(2, '0');
  return path.join(DATA_DIR, `delivery_${year}_${mo}.json`);
}

const CREDIT_FUEL = new Set(['휘발유', '경유', '등유', '유록스']);

function applyDelivery(vendors, deliveries) {
  if (!deliveries.length) return vendors;

  const deliveryMap = new Map();
  deliveries.forEach(d => {
    const key = `${d.vendor}|${d.date}|${d.product}|${d.qty}`;
    deliveryMap.set(key, d);
  });

  return vendors.map(v => {
    const txs = v.txs.map(t => {
      if (!t.vehicle) {
        const match = deliveryMap.get(`${v.name}|${t.date}|${t.product}|${t.qty}`);
        if (match) return { ...t, vehicle: match.vehicle || '배달' };
      }
      return t;
    });

    const baseErrors = v.errors.filter(e => e.type !== 'no_vehicle');
    const noVehicleErrors = txs
      .filter(t => !t.vehicle && CREDIT_FUEL.has(t.product) && t.date >= '2026/06')
      .map(t => ({ type: 'no_vehicle', date: t.date, product: t.product, qty: t.qty, amount: t.amount }));

    const errors = [...baseErrors, ...noVehicleErrors];
    return { ...v, txs, errors, hasError: errors.length > 0 };
  });
}
...
app.post('/api/parse-excel', upload.single('file'), (req, res) => {
  try {
    ...
    const deliveries = readJSON(deliveryFile(year, month), []);
    res.json({ ok: true, vendors: applyDelivery(vendors, deliveries) });
  } catch (e) {
...
// ── 배달 거래내역 업로드 ──────────────────────────────────────
app.post('/api/upload-delivery', upload.single('file'), (req, res) => {
  try {
    if (!req.file) return res.json({ ok: false, error: '파일 없음' });
    const year  = req.body.year  || new Date().getFullYear();
    const month = req.body.month || new Date().getMonth() + 1;
    const { parseDelivery } = require('./lib/deliveryParser');
    const deliveries = parseDelivery(req.file.path);
    writeJSON(deliveryFile(year, month), deliveries);
    const vendors = readJSON(vendorFile(year, month), []);
    res.json({ ok: true, count: deliveries.length, vendors: applyDelivery(vendors, deliveries) });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    const rawVendors = readJSON(vendorFile(year, month), []);
    const deliveries = readJSON(deliveryFile(year, month), []);
    const vendors    = applyDelivery(rawVendors, deliveries);
